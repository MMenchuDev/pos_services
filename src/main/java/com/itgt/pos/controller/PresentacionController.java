...
import com.itgt.pos.service.PresentacionService;
...
public class PresentacionController {
	...
	PresentacionService service;
	  ...
	  List<Presentacion> dataG = new ArrayList<>();
	  
	  @PostMapping("")
	  public ResponseEntity<?> addItem(@RequestBody Presentacion item) {
	    ResponseEntity<?> response;
	    try {
	      dataG.clear();
	      Presentacion newItem = service.addItem(item);
	      dataG.add(newItem);
	  ...
	  public ResponseEntity<?> getPresentacionById(@PathVariable("id") Long id) {
	    ...
	    try {
	      dataG.clear();
	      Presentacion item = service.getItemById(id);
	      dataG.add(item);
	      if (dataG.size() > 0) {
	        map.put("id", 1);
	  ...
	  @PutMapping("")
	  public ResponseEntity<?> updItem(@RequestBody Presentacion data) {
	    ResponseEntity<?> response;// = new ResponseEntity<>(null);
	    try {
	      dataG.clear();
	      Presentacion newItem = service.updItem(data);
	      dataG.add(newItem);
	      mapG.put("id", 1);
