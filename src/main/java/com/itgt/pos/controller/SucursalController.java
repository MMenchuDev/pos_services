...
import com.itgt.pos.service.SucursalService;
...
public class SucursalController {
	...
	List<Sucursal> dataG = new ArrayList<>();
	
	  @PostMapping("")
	  public ResponseEntity<?> addItem(@RequestBody Sucursal item) {
	    ResponseEntity<?> response;
	    try {
	      dataG.clear();
	      Sucursal newItem = service.addItem(item);
	      dataG.add(newItem);
	  ...
	  @GetMapping("id/{id}")
	  public ResponseEntity<?> getSucursalById(@PathVariable("id") Long id) {
	    HashMap<String, Object> map = new HashMap<String, Object>();
	    try {
	      dataG.clear();
	      Sucursal item = service.getItemById(id);
	      dataG.add(item);
	      if (dataG.size() > 0) {
	        map.put("id", 1);
	  ...
	  @PutMapping("")
	  public ResponseEntity<?> updItem(@RequestBody Sucursal data) {
	    ResponseEntity<?> response;// = new ResponseEntity<>(null);
	    try {
	      dataG.clear();
	      Sucursal newItem = service.updItem(data);
	      dataG.add(newItem);
	      mapG.put("id", 1);
